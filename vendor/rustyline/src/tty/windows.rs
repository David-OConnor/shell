...
fn read_input(handle: HANDLE, max_count: u32) -> Result<KeyEvent> {
    ...
    loop {
        ...
        let key_code = match key_event.wVirtualKeyCode {
            ...
            _ => {
                if utf16 == 0 {
                    // LOCAL PATCH (shell): the console produces no translated
                    // character for Ctrl+digit, so upstream drops the event
                    // here. Recover the digit from the virtual key code
                    // (VK '0'..'9' = 0x30..0x39) so Ctrl+digit is bindable.
                    if mods.contains(M::CTRL)
                        && (0x30..=0x39).contains(&key_event.wVirtualKeyCode)
                    {
                        K::Char(char::from(key_event.wVirtualKeyCode as u8))
                    } else {
                        continue;
                    }
                } else {
